...
<script>
    ...
    axios.get(`/api/orders/${orderId}/detailss`)
        .then(response => {
            $('#orderDetails').html(response.data.html);
        });
}
...
function loadCustomerOrders(customerId) {
    $('#orderDetails').empty();
    $('#lstOrders').empty();
    axios.get(`/api/customers/${customerId}/orderss`)
        .then(response => {
            $('#lstOrders').html(response.data.html);
            $('#customerListCollapse').collapse('hide');
        });
}

function displayCustomersList(searchTerm) {
    $('#lstOrders').empty();
    $('#orderDetails').empty();
    axios.get(`/api/customers/search/${searchTerm}`)
        .then(response => {
            $('#lstCustomers').html(response.data.html);
        });
}
...
$(document).ready(function() {
    $('#customer-search-form').on('submit', function(e) {
        ...
        displayCustomersList(searchTerm);
    });
});
